...
class DerivedCryptoFieldsMixin:
    def to_internal_value(self, data):
        derived = set(self.Meta.read_only_fields) - {"group_details", "remaining_days", "authority_details"}
        derived.update({"certificate", "private_key", "csr", "chain_certificates"} if self.Meta.model is Bundle else ())
        supplied = derived.intersection(data)
        if supplied:
            raise serializers.ValidationError({name: "This field is calculated from cryptographic material and is read-only." for name in supplied})
        return super().to_internal_value(data)


class CertificateSerializer(DerivedCryptoFieldsMixin, VisibleRelationshipsMixin, NetBoxModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = (
            ...
            "key_size", "curve", "is_ca", "parent_certificate", "supersedes", "remaining_days", "group_details",
        )
    ...
    def validate(self, attrs):
        ...
        if material:
            ...
            if self.instance is not None and parsed.metadata["fingerprint_sha256"] != self.instance.fingerprint_sha256:
                raise serializers.ValidationError({"material": "Stored material is immutable. Import replacement material as a new object."})
...
class PrivateKeySerializer(DerivedCryptoFieldsMixin, VisibleRelationshipsMixin, NetBoxModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if material:
            ...
            if self.instance is not None and parsed.metadata["public_key_fingerprint"] != self.instance.public_key_fingerprint:
                raise serializers.ValidationError({"key_material": "Stored material is immutable. Import replacement material as a new object."})
...
class CSRSerializer(DerivedCryptoFieldsMixin, VisibleRelationshipsMixin, NetBoxModelSerializer):
...
class BundleSerializer(DerivedCryptoFieldsMixin, VisibleRelationshipsMixin, NetBoxModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = ("identity_fingerprint", "source_filename", "archive_format", "status", "certificate", "private_key", "csr", "chain_certificates", "group_details")
    def _visible(self, obj):
